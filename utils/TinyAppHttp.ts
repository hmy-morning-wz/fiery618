...
function getSystemInfoSync(): Promise<SystemInfo> {
    return new Promise(async (resolve, reject) => {
        let deviceId=""
        let ret: any = await getStorageSync({ key: 'mtr-mdap' })        
        if (ret && ret.data) {
            deviceId = ret.data
        } else {
            deviceId = guid()
               let globalData = getApp().globalData
               if (globalData) { globalData.set('mtr-mdap', deviceId) }
                  else {
                     my.setStorage({ key: 'mtr-mdap', data: deviceId })
                }            
        }
        let systemInfo: any
        let count = 0
        ...
        my.getSystemInfo({
            success: async (res: my.IGetSystemInfoSuccessResult) => {
                ...
                systemInfo.deviceId =deviceId
                //@ts-ignore
                if(!my.$mySystemInfo)  my.$mySystemInfo= res
                resolve(systemInfo)
            },
            fail: () => {
                reject()
