...
function handleNavigateScheme({url}:any,option?:any):boolean{
       if(!url) {
         return false
       } 
       let navigate:any = {}
...
export default {
  ...
  getSystemInfoSync: async():Promise<any> => {
    ...
    if(my.$mySystemInfo) {
       //@ts-ignore
      systemInfo.data = my.$mySystemInfo
      break
    }
    if(systemInfo.data) {
      break
  ...
  if(systemInfo.data) {
       console.log("getSystemInfoSync from systemInfo")
      return Promise.resolve(systemInfo.data)
  }
    return new Promise((resolve, reject) => {
  ...
  handleNavigate: async (options: {url?:string,redirectTo?:any,url_type:string,url_path:string,url_data:any,appId?:string,url_remark:string,publicBizType?:string,publicId?:string }, gd?:GlobalData) => {
...
if (src.indexOf('aliyuncs.com')>-1 || src.indexOf('images.allcitygo.com')>-1){
  ...
  if(systemInfo) {
      let screenWidth = (systemInfo.screen && systemInfo.screen.width) ||  systemInfo.screenWidth
