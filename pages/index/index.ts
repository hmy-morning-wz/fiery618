 ...
 hideLoading(){ 
  ...
  onShareAppMessage(options) {
    ...
    return {
      title: `分享活动`,
      //desc: 'View 组件很通用',
      path: `pages/index/index?sharerId=${userId}`,//?url=${encodeURIComponent(url)} `,
      'web-view': url,
    };
  },
  ...
  async onDrawTap(obj:any) {//红包领取点击
     ...
     */
      ...
      if( taskList2.length && count) { //有次数，翻倍
   ...
   onGiveupDouble(){
      ...
      let  { redBagDouble: {amount,count},gameZone} = this.data
       let showPopInfo= {
            isShowPop: true,
            showPop: "redPacket1",   
            redEnvelopeAmount:amount ||  gameZone.money,              //todo 产品需求这里要写死
        }
  ...
  onTaskComple(data: { code: string, sellerId: string, type: string,taskId:string }) {
    console.log("onTaskComple", data)
    this.notifyListener && this.removeNotifyListener()
    if (!data) {
      return
    }
    let { type, sellerId, code,taskId } = data//,type:"member"
    if (code !== "SUCCESS" || !sellerId) {
      console.log("onTaskComple not success", code)
      return
    }
    let {redBag} = this.data
    let playload: any = { sellerId, sharerId: this.data.sharerId, taskType: type,redBagId:redBag.redBagId }
    // 判断 
    //入会，type:"member"
    //到店，
    //关注
